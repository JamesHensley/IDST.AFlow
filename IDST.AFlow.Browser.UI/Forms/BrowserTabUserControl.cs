    ...
    public partial class BrowserTabUserControl : UserControl
    {
        ...
        public BrowserTabUserControl(Action<string, int?> openNewTab, string url, bool multiThreadedMessageLoopEnabled)
        {
            ...
            browser.JavascriptObjectRepository.ResolveObject += (sender, e) =>
            {
                ...
                {
                    case "boundAsync2":
                        repo.Register("boundAsync2", new AsyncBoundObject(), options: BindingOptions.DefaultBinder, isAsync: true);
                        break;
                    case "boundEvent":
                        var eo = new ScriptedMethodsBoundObject();
                        eo.EventArrived += (string evName, object evObj) => {
                            System.Diagnostics.Debugger.Break();
                        };
                        repo.Register("boundEvent", eo, options: BindingOptions.DefaultBinder, isAsync: true);
                        break;
                    default:
                        break;
                }
