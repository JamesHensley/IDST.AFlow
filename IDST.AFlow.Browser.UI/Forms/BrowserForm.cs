    ...
    public partial class BrowserForm : Form
    {
        ...
        private async void PrintToPdfToolStripMenuItemClick(object sender, EventArgs e)
        {
            ...
            if (control != null)
            {
                ...
                {
                    var success = await control.Browser.PrintToPdfAsync(dialog.FileName, new CefSharp.PdfPrintSettings
                    {
                        MarginType = CefSharp.CefPdfPrintMarginType.Custom,
                        MarginBottom = 10,
                        MarginTop = 0,
                        MarginLeft = 20,
                        MarginRight = 10
        ...
        private void RunFileDialogToolStripMenuItemClick(object sender, EventArgs e)
        {
            ...
            if (control != null)
            {
                control.Browser.GetBrowserHost().RunFileDialog(CefSharp.CefFileDialogMode.Open, "Open", null, new List<string> { "*.*" }, 0, new RunFileDialogCallback());
            }
        }
        ...
        private void WorkflowToolStripMenuItemClick(object sender, EventArgs e) {
            ...
            if (control != null)
            {
                ...
                var initialWorkflowData = new WorkflowData()
                {
                    ...
                    PersistentData = new WorkflowPersistentData() {
                        PageData = new List<KeyValuePair<string, string>>(),
                        FinalList = new List<System.Dynamic.ExpandoObject>(),
